...
meshNodes = []
S = [20]
A = np.linspace(5,20,20) #25
B = np.linspace(0,2,32) #40
###
for s in S:
    for a in A:
...
def parallelBarcodes(node,gr,index):
    ...
    sol[0].plot(title='Final $u$ value at [$\\alpha$,$\\beta$,$\\sigma$] = '+str([round(a,2),round(b,2),round(s,2)]),
                 cmap='turbo', filename=path+'\\pythonfigs\\'+str(index+1)+'.png', action='close')
...
dfNodes.to_csv("CIMAnodes.csv",index=False,header=["a","b","s"])

npStuff = np.asarray(barcodes)

dfBarcodes = pd.DataFrame(npStuff[:,0,:])
dfBarcodes.to_csv("CIMAbarcodes.csv",index=False,header=["u0","v0","u1","v1"],sep=',')

dfMinMax = pd.DataFrame(npStuff[:,1,:])
dfMinMax.to_csv("CIMAminmax.csv",index=False,header=["umin","umax","vmin","vmax"],sep=',')
